...
def getResult(inputSTR, utterance, args, resultDICT):
    ...
    if utterance == "[特選普洱][不要加糖]跟[冰]塊":
        ...
        elif args[2] in userDefinedDICT["sugar"]:
            resultDICT["sugar"].append(args[2])        
        else:
            pass
    ...
    if utterance == "[都][去冰][半糖]":
        if args[2] in userDefinedDICT["sugar"]:
            resultDICT["sugar"].append(args[2])
        elif args[1] in userDefinedDICT["sugar"]:
            resultDICT["sugar"].append(args[1])        
        else:
            pass

    if utterance == "都[不要糖]":
        if args[0] in userDefinedDICT["sugar"]:
            resultDICT["sugar"].append(args[0])     
        else:
            pass
    ...
    if utterance == "[原鄉][兩杯][一杯][半糖][少冰][一杯][全糖][正常冰]":
        for a in args[3:8]:
            if a in userDefinedDICT["sugar"]:
                resultDICT["sugar"].append(a)
        else:
            pass 
